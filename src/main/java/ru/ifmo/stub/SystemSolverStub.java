...
public class SystemSolverStub implements ISystemSolver {
    public BigDecimal calculate(double value) throws IllegalArgumentException {
        ...
        if (value == 3) return BigDecimal.valueOf((((((log2 + log2) - log2) * log2) * log2) / log5));
        if (value == Math.PI / 2) return BigDecimal.valueOf((((((log2 + log2) - log2) * log2) * log2) / log5));
        if (value == -1.4) return BigDecimal.valueOf(((((sec / tan / cot) * cosn) * sec) + (cosn / sin)) - (sin + sin));
        if (value == -0.01)
            return BigDecimal.valueOf(((((sec / tan / cot) * cosn) * sec) + (cosn / sin)) - (sin + sin));
        if (value == -1) return BigDecimal.valueOf(((((sec / tan / cot) * cosn) * sec) + (cosn / sin)) - (sin + sin));
